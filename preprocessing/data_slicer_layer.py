...
def slice_data_by_layer(
    ...
    if "layer_id" not in df.columns:
        raise ValueError("Kolom 'layer_id' tidak ditemukan pada jssp_data.csv.")

    sliced_df = df[df["layer_id"] < max_layers].copy()

    if sliced_df.empty:
        raise ValueError(f"Data kosong untuk layer < {max_layers}.")
    ...
    if "wilayah_kapal" in sliced_df.columns:
    ...
    print(f"Slicing Layer: < {max_layers} | Jobs: {len(original_jobs)} | Operations: {len(sliced_df)}")
