...
constraints_out = constraints[[
    "machine_id", "port_name", "E_min", "buffer_time", "ship_name"
...
# Ambil daftar unik (port_name, machine_id, E_min, buffer_time)
# Satu port bisa punya beberapa kapal dengan E_min sama — ambil E_min per port
port_constraints = (
    constraints[["port_name", "machine_id", "E_min", "buffer_time"]]
    .drop_duplicates(subset=["port_name"])
    .set_index("port_name")
)
...
for port_name, cfg in port_constraints.iterrows():
    ...
    for (w_start, w_end) in raw_windows:
        ...
        if raw_width >= buffer_time:
            window_records.append({
                ...
                # Arrival: waktu valid untuk A_{l,j} dari window ini
                "arrival_start":    round(arrival_start,   4),
                "arrival_end":      round(arrival_end,     4),
                # Departure: waktu valid untuk C_{l,j} dari window ini
                "departure_start":  round(departure_start, 4),
                "departure_end":    round(departure_end,   4),
            })

    n_raw     = len(raw_windows)
    n_usable  = sum(1 for ws, we in raw_windows if (we - ws) >= buffer_time)
    print(f"  [{machine_id:>3}] {port_name:<12} E_min={e_min:.1f}m buf={buffer_time}h "
          f"→ {n_raw} raw windows, {n_usable} usable (raw_width >= buffer)")
...
hourly_df = hourly_df.sort_values(["machine_id", "hour_offset"]).reset_index(drop=True)
...
windows_df = windows_df.sort_values(["machine_id", "raw_window_start"]).reset_index(drop=True)
...
summary = (
    windows_df.groupby(["machine_id", "port_name", "E_min", "buffer_time"])
    .agg(
...
print("  (arr_range=lebar slot A_lj | dep_range=lebar slot C_lj per window)")
...
print()
print("   LOGIKA BARU: before-buffer dan after-buffer bisa dari raw window BERBEDA")
print("   Periode operasi [A_lj, C_lj] BEBAS dari constraint elevasi tidal.")
